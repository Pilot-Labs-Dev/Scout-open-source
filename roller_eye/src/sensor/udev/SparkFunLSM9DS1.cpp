...
#define NOT_SUPPORT()	do{std::cout<<"not support yet"<<std::endl;plt_assert(false);}while(0);

float magSensitivity[4] = {0.00014, 0.00029, 0.00043, 0.00058};

LSM9DS1::LSM9DS1():fd(-1)
{
	init(IMU_MODE_I2C, LSM9DS1_AG_ADDR(0), LSM9DS1_M_ADDR(0));
}

LSM9DS1::LSM9DS1(interface_mode interface, uint8_t xgAddr, uint8_t mAddr):fd(-1)
{
...
{
	static float gyroSensitive[]={8.75e-3,17.50e-3,70e-3};
	switch (settings.gyro.scale)
	{
	case 245:
		gRes=gyroSensitive[0];
		break;
	case 500:
		gRes=gyroSensitive[1];
		break;
	case 2000:
		gRes=gyroSensitive[2];
		break;
	default:
		plt_assert(false);
		break;
...
{
	static float accSensitive[]={0.061e-3,0.122e-3,0.244e-3,0.732e-3};
	switch (settings.accel.scale)
	{
	case 2:
		aRes=accSensitive[0];
...
{
	plt_assert(change_file_mode(I2C_DEV_PATH,"666")==0);
...
void LSM9DS1::I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
	...
	plt_assert(ret==0);
	ret=i2c_smbus_write_byte_data(fd, subAddress,data);
	plt_assert(ret==0);
}
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
	int ret=ioctl(fd, I2C_SLAVE, address);
	plt_assert(ret==0);
	ret=i2c_smbus_read_byte_data(fd, subAddress);
	plt_assert(ret>=0);	
	return	(uint8_t)ret;
}
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	int ret=ioctl(fd, I2C_SLAVE, address);
	plt_assert(ret==0);
	ret=i2c_smbus_read_i2c_block_data(fd,subAddress,count,dest);
	plt_assert(ret==(int)count);
	return ret;
}
