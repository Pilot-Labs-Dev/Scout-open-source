...
public:
    ...
    {    
        ...
        updateTranslate();
        //base frame:x->right,y->front,z->up
...
private:
    ...
    void loadTune()
    {
        ...
        ROS_INFO("motor tune info:delta=%f",mTuneDelta);
        for(auto t:mTunes){
            ROS_INFO("%f",t);
        }
    out:
...
public:
    ...
    mVw(0)
    {
        ...
        resetAutoIMUCalibra();
        setupMotor();
        //mCmdVel=mHandle.subscribe("cmd_vel", 100, &MotorDriver::volecityCallback,this);
        ...
        // mTimer=mHandle.createTimer(ros::Duration(6),&MotorDriver::timerCallback,this);
        // mTimer.stop();
    }
    ~MotorDriver()
    {
...
private:
...
#if 0
        if(mDistance<OBSTACLE_MIN_DISTANCE){
            ymax=0.0;
        }else if(mDistance<2*OBSTACLE_MIN_DISTANCE ){
   ...
   void setupMotor()
    ...
    void autoCalibraIMU(double x,double y,double w)
    {
        if (mBPatrolCalib && mIMUCalibraCnt<IMU_CALIBRA_CNT){
            ...
            if(++mIMUCalibraCnt==IMU_CALIBRA_CNT){
                ...
                mBPatrolCalib = false;
                ROS_INFO("new calibration by patrol a:%f,%f,g:%f",mCalibra.x(),mCalibra.y(),mCalibra.z());
            }
            ...
            if(abs(w)<GYRO_CALIBRA_THRESHOLD && 
                ...
                if(++mIMUCalibraCnt==IMU_CALIBRA_CNT){
                    mCalibra+=mIMUCalibraOffset/IMU_CALIBRA_CNT;
                    ROS_INFO("new calibration a:%f,%f,g:%f",mCalibra.x(),mCalibra.y(),mCalibra.z());
                }
            }else{
                resetAutoIMUCalibra();
            }
    ...
    {
        ROS_INFO("call imuPatrolCalibStatus %d", mIMUCalibraCnt);    
...
public:
    ...
    void start()
    {
        ...
        ROS_INFO("disable motor dynamic adjust");
    }
    void stop()
    {
        g_motor_driver->enable_adjust(true);
        ROS_INFO("enable motor  dynamic adjust");
    }
...
public:
    ...
    void start()
    {
        ...
        ROS_INFO("enable motor direction fix mode");
    }
    void stop()
    {
        g_motor_driver->enable_dir_fixed(false);
        ROS_INFO("disable motor direction fix mode");
    }
    int setData(std_msgs::String& s)
    {
