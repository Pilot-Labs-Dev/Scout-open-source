...
static void checkTestBmpDir(const char *dirname) {
  ...
  if (flag != 0) {
    //std::cout<<"create bmp folder failed: "<< dirname << std::endl;
  }
}
...
{
  ROS_INFO("ObjDetectPub::ObjDetectPub %s", name.c_str());
...
{
  ROS_INFO("ObjDetectPub::onParamChanged >>>%d, %d, %d, 0x%x",
  ...
  if (!resume){
    ...
    stopPublish();
    video_stream_destory(vsHandle);
    ROS_INFO("ObjDetectPub::onParamChanged >>> stop END");
  }else {
    mNeedResume = false;
    vsHandle = mCameraHandle->createVideoStream(1);
    startPublish();
    ROS_INFO("ObjDetectPub::onParamChanged >>> start END");
  }
}
    ...
    if (outputs.size() >= 4) {
        ...
        if(maxObjIdx>=0){          
           ...
           // Get class
            ...
            //LOG(INFO) << score << ": " << cls << " " << label << " [";
            //LOG(INFO) << top << ", "<< left << ", " << bottom << ", " << right << "]\n";
            ...
            ROS_INFO("found obj name: %s maxObjIdx:%d", objMsg.name.c_str(), maxObjIdx);
...
void ObjDetectPub::tfliteInit(Settings *s)
{
  ...
  ROS_INFO("found model file: %s", s->model_name.c_str());
  if (access(s->labels_file_name.c_str(), 0) != 0) {
    ROS_FATAL("Not found label file: %s", s->labels_file_name.c_str());
    return;
  }
 ROS_INFO("found label file: %s", s->labels_file_name.c_str());
  ...
  // if (!s->test_bmp_dir.empty()) {
  //   checkTestBmpDir(s->test_bmp_dir.c_str());
  // }
