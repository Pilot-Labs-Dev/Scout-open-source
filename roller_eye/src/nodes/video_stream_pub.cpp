...
{
  ROS_INFO("VideoStreamPub::onParamChanged >>>%d, %d, %d, 0x%x",
  ...
  if (!resume){
    ...
    stopPublish();
    video_stream_destory(vsHandle);
    ROS_INFO("VideoStreamPub::onParamChanged >>> stop END");
  }else {
    mNeedResume = false;
    mSessionH264++;
    ...
    startPublish();
    ROS_INFO("VideoStreamPub::onParamChanged >>> start END");
  }
}
...
{
  if(!m_pubRunning) {
    //std::cout << "publishLoop already stopped" << std::endl;
    return;
  }
...
{
  //std::cout << "VideoStreamPub::publishLoop" << std::endl;
  FrameBuff *frameBuff;
  ...
  while (m_pubRunning) {
    ...
    if (mBldFrame.data != nullptr) {
      if (fmt == V4L2_PIX_FMT_YUYV) {
        ...
        // GraphicUtils::getInstance()->rgaDrmBlend(mBldFrame.data, mBldFrame.width, mBldFrame.height, RK_FORMAT_YCbCr_420_P, (uint8_t*)frameBuff->addr, frameW, frameH, RK_FORMAT_YCbCr_420_P);
        // GraphicUtils::getInstance()->rgaDrmBlend(mBldFrame.data, mBldFrame.width, mBldFrame.height, RK_FORMAT_YCrCb_420_SP, (uint8_t*)frameBuff->addr, frameW, frameH, RK_FORMAT_YCrCb_420_SP);
      }
    }
    if (mObjDet.score > 0) {
...
/*
void VideoStreamPub::publishRaw(Frame *tmpFrame)
{
  static uint32_t seq = 0;
  roller_eye::frame msgFrame;
  msgFrame.seq = ++seq;
  msgFrame.session = 1001; //todo
  msgFrame.stamp = tmpFrame->getTimestamp();
  msgFrame.type = msgFrame.VIDEO_STREAM_JPG; //fixme
  msgFrame.par1 = getCaptureParam().sInfo.fInfo.width;
  msgFrame.par2 = getCaptureParam().sInfo.fInfo.height;
  msgFrame.par3 = 0;
  msgFrame.data.clear();
  msgFrame.data.insert(msgFrame.data.end(), &tmpFrame->getData()[0], &tmpFrame->getData()[tmpFrame->getSize()]);

  //mPublisher.publish(msgFrame);
}
*/
...
void VideoStreamPub::publishH264(Frame *tmpFrame)
{
  ...
  //if (mObjDet.score > 0)
  //  dynamic_cast<EncoderH264*>(mEncoder.get())->draw_rect(mObjDet.left, mObjDet.top, mObjDet.right - mObjDet.left, mObjDet.bottom - mObjDet.top, 255, 0, 0);
