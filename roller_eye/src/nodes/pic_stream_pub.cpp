...
{
  ROS_INFO("PicStreamPub::onParamChanged >>>%d, %d, %d, 0x%x",
  ...
  if (!resume){
    ...
    stopPublish();
    video_stream_destory(vsHandle);
    ROS_INFO("PicStreamPub::onParamChanged >>> stop END");
  }else {
    mNeedResume = false;
    mSessionJPG++;
    mEncoder.reset(new EncoderJPEG(&param->sInfo));
    ...
    startPublish();
    ROS_INFO("PicStreamPub::onParamChanged >>> start END");
  } 
}
...
{
  //std::cout << "PicStreamPub::publishLoop" << std::endl;
  FrameBuff *frameBuff;
