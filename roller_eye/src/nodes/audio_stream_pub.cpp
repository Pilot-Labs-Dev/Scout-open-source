...
AudioStreamPub::~AudioStreamPub()
{
	...
	mEncoder = nullptr;
	//std::cout << "~AudioStreamPub" << std::endl;
}
...
//	m_paMutex.lock();
	...
	//std::cout << "AudioStreamPub::flush: " << m_out << "/" << m_in << std::endl;
	m_out = m_in;
//	m_paMutex.unlock();
}
...
{
	//std::cout << "AudioStreamPub::captureLoop" << std::endl;
	int error = 0;

	//int ret = arecord_init(SND_PCM_FORMAT_S16_LE, 2, 22050);
...
{
	if(!m_capRunning) {
		//std::cout << "captureLoop already stopped" << std::endl;
        return;
	}
	//std::cout << " *** THREAD JOINING: captureLoop ***" << std::endl;
...
void AudioStreamPub::stopPublish()
{
	...
	//std::cout << " *** THREAD JOINING: publishLoop ***" << std::endl;
	m_dataMutex.lock();
	m_pubRunning = false;
	m_dataMutex.unlock();
	m_dataTrigger.notify_all();
...
{
	//std::cout << "AudioStreamPub::publishLoop" << std::endl;
